...
#obsolete algorithm
# Check if ZRAM is active to avoid creating a wrong GRUB or initramfs that could make the system unbootable.
...
echo "Backing up the systemd‑zram‑setup@.service file and deactivating it"
sudo cp -r /usr/lib/systemd/system/systemd-zram-setup@.service /usr/lib/systemd/system/systemd-zram-setup@.service.bak
sudo echo "" > /usr/lib/systemd/system/systemd-zram-setup@.service;
...
echo "Restore original swap-zram service configuration..."
swap_ram_bak="/usr/lib/systemd/system/systemd-zram-setup@.service.bak"
if [[ ! -e "$swap_ram_bak" ]]; then
    echo "⚠️  Warning: $swap_ram_bak does not exist."
    echo "The script cannot continue without this backup file."
    sleep 2;
    exit 1   # non‑zero exit code signals failure
fi
...
# If we reach this point the file exists
echo "✅  $swap_ram_bak found. Continuing with the program..."
#restore .bak of swap-zram service
sudo cp -r /usr/lib/systemd/system/systemd-zram-setup@.service.bak /usr/lib/systemd/system/systemd-zram-setup@.service;
echo;
